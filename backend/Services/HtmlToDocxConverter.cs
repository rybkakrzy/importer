...
public class HtmlToDocxConverter
{
    ...
    private List<OpenXmlElement> ConvertListElement(HtmlNode node, bool ordered, int level = 0)
    {
        ...
        foreach (var child in node.ChildNodes)
        {
            ...
            // Wcięcie odpowiednie do poziomu
            var leftIndent = (720 * (level + 1)).ToString();
            props.Append(new Indentation { Left = leftIndent, Hanging = "360" });
    ...
    private int CreateAbstractNumbering(bool ordered)
    {
        ...
        for (int lvl = 0; lvl < 9; lvl++)
        {
            ...
            else
            {
                ...
                levelDef.Append(new LevelText { Val = bulletChar });
