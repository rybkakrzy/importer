...
public class SaveDocumentCommandHandler : IRequestHandler<SaveDocumentCommand, Result<SaveDocumentResult>>
{
    ...
    {
        var docxBytes = _converter.Convert(request.Html, request.Metadata, request.Header, request.Footer, request.Margins, request.PageSize, request.SectionHeadersFooters, request.Footnotes);
