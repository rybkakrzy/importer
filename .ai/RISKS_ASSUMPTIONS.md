...
| R-23 | **Forma claimu `groups`** (ADR-0012): Entra domyślnie emituje object-id (GUID), Doc2 `RolesOptions.GroupNames` używa nazw `KUTAS_200_*` → bez optional-claim „groups" (nazwy) mapowanie nie zadziała. | Medium | Medium | Skonfigurować optional claim (cloud display name / sAMAccountName) albo wpisać GUID-y w `RolesOptions`. | Open |
