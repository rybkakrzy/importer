...
- **Uwierzytelnianie:** Entra ID przez **`Microsoft.Identity.Web`** (`AddMicrosoftIdentityWebApi`, sekcja `AzureAd` — `ClientId`/`TenantId`/`Instance`/`Audience`). **Cała aplikacja za logowaniem** — `[Authorize]` na `BaseApiController`; Health anonimowy. Front: MSAL (redirect), token dołączany `MsalInterceptor`. **Keycloak (legacy) dual-auth** (gdy `Keycloak:Enabled`): drugi schemat JwtBearer + policy scheme `EntraOrKeycloak` wybierający po issuerze tokena. Patrz ADR-0012.
