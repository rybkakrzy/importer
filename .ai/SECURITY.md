...
- **`CorporateKey` z access tokena** — `ClaimsCurrentUserProvider` czyta konfigurowalny claim (`AzureAd:CorporateKeyClaim`, domyślnie **`corpKey`**) **case-insensitive po nazwie claimu** (token ING-AD niesie `corpkey` małymi literami, a `CaseSensitiveClaimsIdentity` z Identity.Web nie dopasowałby `FindFirst`); brak/pusty → null → dokument ograniczony = **403**. corpKey NIE jest transportowany z GUI — jedynym źródłem jest token (ADR-0021). (Dev: `HttpHeaderCurrentUserProvider` z nagłówka `X-Corporate-Key`, fallback `DEV-LOCAL` — nie rejestrowany w produkcji.)
