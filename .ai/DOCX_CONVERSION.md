...
| Przypisy dolne (footnotes) | Implemented | R+W | R: `RenderFootnoteReference`, `ExtractFootnotes`; W: `AddFootnotes`, `CreateFootnoteReferenceRun`, `BuildFootnoteElement`; model `DocumentContent.Footnotes` | ADR-0032 (2026-07-10): `w:footnoteReference`↔`<sup class="footnote-ref" data-footnote-id>`; treść w bocznym `Footnotes` (jedno źródło prawdy, konwertowana istniejącymi konwerterami akapitu/tabeli — formatowanie/wieloakapitowość/Unicode). Tożsamość `fn-<ooxmlId>` stabilna; numer widoczny = kolejność pierwszych odwołań; `w:id` OOXML deterministyczny 1..N na eksporcie (separatory techniczne -1/0). Writer tworzy `word/footnotes.xml`+relację+content type; dokument bez przypisów nie dostaje części. GUI: panel treści poza contenteditable + add/edit/delete/renumeracja. NIE: przypisy końcowe (endnotes), `w:footnotePr` (format numeracji/restart), przypisanie do konkretnej strony, ścieżka Sign |
